...
from shopping_agent import agent, model_base_guardrail
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "Upload product image", type=["jpg", "jpeg", "png", "webp", "avif"]
    )

    if uploaded_file:
        st.image(uploaded_file, width="stretch")

    if uploaded_file and st.button("Find similar products", width="stretch"):
...
# ---------------------------------------------------------------------------
# Run agent, model_base_guardrail if there's an unprocessed message (image upload triggers this)
# ---------------------------------------------------------------------------
if (
    st.session_state.messages
...
if prompt := st.chat_input("e.g. I want organic honey under $15 with 4+ rating"):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking…"):
            relevent_query = model_base_guardrail(st.session_state.messages)
            print("\n\n classificaton \n\n", relevent_query)
            if relevent_query == "proceed":
                result = agent.invoke({"messages": st.session_state.messages})
                response = result["messages"][-1].content.replace("`", "")
            else:
                response = "Query is not related to shopping " + relevent_query
        st.markdown(response.replace("$", r"\$"))
